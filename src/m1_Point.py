...
class Point(object):
    ...
    def __init__(self, x, y):
        """ init gives values to instance variables """
        self.x = x
        self.y = y
        self.count = 0
        self.start_x = x
    ...
    def clone(self):
        clone = Point(self.x, self.y)
        return clone

    def move_to(self, dx, dy):
        pyth = (self.x - dx)**2 + (self.y - dy)**2
        dist = math.sqrt(pyth)
        self.dis = self.dis + dist
        self.x = dx
        self.y = dy
    ...
    def get_distance_from(self, another_point):
        pyth = (self.x - another_point.x)**2 + (self.y - another_point.y)**2
        dist = math.sqrt(pyth)
        return dist

    def get_distance_from_start(self):
        pyth = (self.x - self.start_x)**2 + (self.y - self.start_y)**2
        dist = math.sqrt(pyth)
        return dist
    ...
    def halfway_to(self, p2):
        new_point = Point(0, 0)
        new_point.x = (self.x + p2.x) / 2
        new_point.y = (self.y + p2.y) / 2
        return new_point
